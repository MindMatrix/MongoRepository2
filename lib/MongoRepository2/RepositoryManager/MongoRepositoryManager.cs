namespace MongoRepository2
{
        ...
        where T : IEntity<TKey>
    {
        ...
        {
            return this.collection.Database.ListCollections(new ListCollectionsOptions { Filter = new BsonDocument("name", this.Name) }).Any();
        }
        ...
        {
            var options = new ListCollectionsOptions { Filter = new BsonDocument("name", this.Name) };
            return await (await this.collection.Database.ListCollectionsAsync(options)).AnyAsync();
        }
        ...
        /// Drops the collection.
        /// </summary>
        public async virtual Task DropAsync()
        {
        ...
        /// </summary>
        /// <returns>Returns true when the repository is capped, false otherwise.</returns>
        public async virtual Task<bool> IsCappedAsync()
        {
            return (await this.GetStatsAsync()).IsCapped;
        }
        ...
        /// </summary>
        /// <param name="keyname">The name of the indexed field.</param>
        public async virtual Task DropIndexAsync(string keyname)
        {
            await this.DropIndexesAsync(new string[] { keyname });
        }
        ...
        /// </summary>
        /// <param name="keynames">The names of the indexed fields.</param>
        public async virtual Task DropIndexesAsync(IEnumerable<string> keynames)
        {
            foreach (var k in keynames)
                await this.collection.Indexes.DropOneAsync(k);
        }
        ...
        /// Drops all indexes on this repository.
        /// </summary>
        public async virtual Task DropAllIndexesAsync()
        {
        ...
        /// Index will be ascending order, non-unique, non-sparse.
        /// </remarks>
        public async virtual Task EnsureIndexAsync(string keyname)
        {
            await this.EnsureIndexesAsync(new string[] { keyname });
        }
        ...
        /// This is a convenience method for EnsureIndexes(IMongoIndexKeys keys, IMongoIndexOptions options).
        /// </remarks>
        public async virtual Task EnsureIndexAsync(string keyname, bool descending, bool unique, bool sparse)
        {
            await this.EnsureIndexesAsync(new string[] { keyname }, descending, unique, sparse);
        }
        ...
        /// Index will be ascending order, non-unique, non-sparse.
        /// </remarks>
        public async virtual Task EnsureIndexesAsync(IEnumerable<string> keynames)
        {
            await this.EnsureIndexesAsync(keynames, false, false, false);
        }
        ...
        /// This is a convenience method for EnsureIndexes(IMongoIndexKeys keys, IMongoIndexOptions options).
        /// </remarks>
        public async virtual Task EnsureIndexesAsync(IEnumerable<string> keynames, bool descending, bool unique, bool sparse)
        {
            foreach (var k in keynames)
            {
                var opt = new CreateIndexOptions { Unique = unique, Sparse = sparse };
                var builder = Builders<T>.IndexKeys;
                if (descending)
                    await this.collection.Indexes.CreateOneAsync(builder.Descending(k), opt);
                else
                    await this.collection.Indexes.CreateOneAsync(builder.Descending(k), opt);
            }
        }
        ...
        /// <param name="keyname">The indexed fields.</param>
        /// <returns>Returns true when the indexes exist, false otherwise.</returns>
        public async virtual Task<bool> IndexExistsAsync(string keyname)
        {
            return await this.IndexesExistsAsync(new string[] { keyname });
        }
        ...
        /// <param name="keynames">The indexed fields.</param>
        /// <returns>Returns true when the indexes exist, false otherwise.</returns>
        public async virtual Task<bool> IndexesExistsAsync(IEnumerable<string> keynames)
        {
            var ix = await (await this.collection.Indexes.ListAsync()).ToListAsync();
            return keynames.All(k => ix.Contains(BsonValue.Create(k)));
        }
        ...
        /// Runs the ReIndex command on this repository.
        /// </summary>
        public async virtual Task ReIndexAsync()
        {
            await this.collection.Database.RunCommandAsync(new BsonDocumentCommand<BsonDocument>(new BsonDocument { { "reIndex", this.Name } }));
        }
        ...
        /// <returns>Returns total size for the repository (data + indexes).</returns>
        [Obsolete("This method will be removed in the next version of the driver")]
        public async virtual Task<long> GetTotalDataSizeAsync()
        {
            return (await this.GetStatsAsync()).DataSize;
        }
        ...
        /// <returns>Returns total storage size for the repository (data + indexes).</returns>
        [Obsolete("This method will be removed in the next version of the driver")]
        public async virtual Task<long> GetTotalStorageSizeAsync()
        {
            return (await this.GetStatsAsync()).StorageSize;
        }
        ...
        /// <returns>Returns a ValidateCollectionResult.</returns>
        /// <remarks>You will need to reference MongoDb.Driver.</remarks>
        public async virtual Task<ValidateCollectionResult> ValidateAsync()
        {
            return new ValidateCollectionResult(
                await this.collection.Database.RunCommandAsync(new BsonDocumentCommand<BsonDocument>(new BsonDocument { { "validate", this.Name } }))
            );
        }
        ...
        /// <returns>Returns a CollectionStatsResult.</returns>
        /// <remarks>You will need to reference MongoDb.Driver.</remarks>
        public async virtual Task<CollectionStatsResult> GetStatsAsync()
        {
            return new CollectionStatsResult(
                await this.collection.Database.RunCommandAsync(new BsonDocumentCommand<BsonDocument>(new BsonDocument { { "collstats", this.Name } }))
            );
        }
        ...
        /// </summary>
        /// <returns>Returns the indexes for this repository.</returns>
        public async virtual Task<List<BsonDocument>> GetIndexesAsync()
        {
            return await (await this.collection.Indexes.ListAsync()).ToListAsync();
        }
    }
